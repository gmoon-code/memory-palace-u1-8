...
def find_repo_root(root):
    ...
    for p in root.rglob("START_HERE.md"):
        parent = p.parent
        if (parent / "content").is_dir() and (parent / "frontend").is_dir() and (parent / "tests").is_dir():
...
def main():
    ...
    if actual != EXPECTED_SHA and not args.allow_unverified_baseline:
        raise RuntimeError(
            f"Baseline checksum mismatch. Expected {EXPECTED_SHA}, got {actual}. "
            "Use --allow-unverified-baseline only after manually confirming the baseline."
        )
    with tempfile.TemporaryDirectory() as td:
        ...
        if manifest.exists():
            try:
                ...
                if not args.allow_unverified_baseline:
                    raise
        for rel in COPY_DIRS:
            src = OVERLAY_ROOT / rel
            if src.exists():
                merge_dir(src, repo / rel)
        # Preserve overlay provenance and application utilities inside the integrated repo.
        shutil.copy2(OVERLAY_ROOT / "BASELINE_U1_U7_F6.json", repo / "content/ap-biology/unit-8/BASELINE_U1_U7_F6.json")
        (repo / "scripts").mkdir(exist_ok=True)
        shutil.copy2(OVERLAY_ROOT / "scripts/apply_unit8_f1_overlay.py", repo / "scripts/apply_unit8_f1_overlay.py")
        shutil.copy2(Path(__file__), repo / "scripts/apply_unit8_f2_overlay.py")
        out.parent.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
            for p in sorted(repo.rglob("*")):
                if p.is_file():
                    z.write(p, Path(repo.name) / p.relative_to(repo))
    print(out)
    print(sha256(out))
