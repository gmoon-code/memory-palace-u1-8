...
def main() -> None:
    ...
    require("APBIO_DIR" not in backend_text, "management still depends on the AP Biology content root")
    require("idx_media_assets_course_status" in backend_text, "staged media database is not course-indexed")
    require("ALTER TABLE media_assets ADD COLUMN course_id" in backend_text, "legacy staged media migration does not add course_id")
    require('"scope": {"course_id": course_id' in backend_text, "portable export bundles do not declare course scope")
    require("Import bundle belongs to course" in backend_text, "import validation does not reject cross-course bundles")
    require("admin_catalog.catalog(course_id)" in backend_text, "search/export/bulk reads are not course-scoped")
    require('SELECT * FROM content_drafts WHERE course_id = ? AND status = \'draft\'' in backend_text, "workspace draft search is not course-scoped")
    ...
    require(route_text.count('course_id: str = Field(default="ap-biology"') >= 8, "management mutation requests do not consistently carry course_id")
    ...
    require("currentAdminCourseCatalogReady()" in frontend_text, "management preview does not use catalog_ready access")
    require("managementModes.has(mode) && currentAdminCourseCatalogReady()" in frontend_text, "read-only catalog-ready courses cannot inspect management views")
    require("currentAdminCourseEditable()" in frontend_text, "management write controls do not respect editable access")
    require("course_id: preview.course_id || currentAdminCourseId()" in frontend_text, "bulk apply does not preserve preview course identity")
    require("course_id: item.course_id || currentAdminCourseId()" in frontend_text, "media mutations do not preserve asset course identity")
    ...
    print("- Question Bank, Review System, Challenge Lab, Media, Import/Export, bulk replace, and search are explicitly course-scoped")
    ...
    print("- media uploads are staged in private server_data, migrated with course identity, and are never published automatically")
    print("- import/export uses course-scoped portable curriculum bundles while authentication and security state remain excluded")
    print("- Step 6 replacement and Step 7 management modules are both loaded in Content Studio")
    print("- published course content and public GitHub Pages remain outside Step 7 write paths")
