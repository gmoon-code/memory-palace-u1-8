...
problems = []
journey = read(U8 / 'journeys/U8-J1.json')
...
lock = read(U8 / 'content-lock-f4a.json')
f3_lock = read(U8 / 'content-lock-f3.json')

expected_loci = [f'U8-L{i:02d}' for i in range(1, 13)]
if journey.get('palace_id') != 'U8-J1': problems.append('Journey 1 ID changed')
...
if 'twelve permanent locations' not in journey.get('route_orientation', '').casefold(): problems.append('route orientation does not define the twelve-location institute route')
...
for s in journey['scenes']:
    ...
    if not any(c.get('name') == 'Transparent stimulus-response tablet' for c in s['cast']): problems.append(f'{lid} lost continuity object')
...
if len(assigned) != 50 or len(set(assigned)) != 50: problems.append(f'Journey 1 knowledge coverage is {len(assigned)} refs and {len(set(assigned))} unique, expected 50 and 50')
if exact_targets != 35: problems.append(f'Journey 1 exact-name target count is {exact_targets}, expected 35')

full = ' '.join(' '.join(s['story_paragraphs']) for s in journey['scenes']).casefold().replace('**', '')
visible = {
    'proximate_vs_ultimate': ['proximate questions ask how a behavior occurs', 'ultimate questions ask why a behavior exists in evolutionary terms'],
    'innate_vs_learned': ['innate behavior develops with strong genetic and developmental specification', 'learned behavior is modified by experience'],
    'fap_qualification': ['fixed action pattern', 'once initiated, are usually carried to completion'],
    'migration_vs_cue': ['migration is a regular, long-distance change in location', 'navigation cues help orient that movement'],
    'kinesis_vs_taxis': ['kinesis is a non-directional change in movement rate or turning frequency', 'taxis is directed movement toward or away from a stimulus'],
    'signal_and_pheromone': ['a signal is a stimulus transmitted from one organism to another', 'a pheromone is a chemical signal released by an organism'],
    'imprinting_guard': ['imprinting is a long-lasting learned behavioral response formed during a sensitive period', 'is not universally just the first individual encountered'],
    'learning_modes': ['spatial learning', 'associative learning', 'social learning'],
    'mating_systems': ['one male mates with multiple females', 'one female mates with multiple males', 'sexual dimorphism'],
    'altruism_inclusive_fitness': ['does not imply conscious selflessness', 'kin selection is natural selection', 'inclusive fitness combines an individual’s direct reproductive success'],
    'phototaxis_vs_phototropism': ['phototaxis was directed movement of a whole organism relative to light', 'phototropism is directional plant growth in response to light'],
    'photoperiod_guard': ['relative lengths of day and night', 'night length is often the critical cue'],
    'hydrangea_guard': ['ph changes aluminum availability', 'context dependent across species and cultivars'],
}
...
# Protect F1 and F2 artifacts and the locked F3 brief layer.
...
if is_later_f5:
    ...
    if status.get('journey_count', 0) < 1 or status.get('scene_count', 0) < 12 or status.get('memory_objects') != 0 or status.get('application_challenges') != 0:
        problems.append('Unit 8 no longer preserves the F4A developer-preview boundary')

required = [
    U8 / 'journeys/U8-J1.json', U8 / 'journeys-f4a.json', U8 / 'status-f4a.json', U8 / 'content-lock-f4a.json', U8 / 'f4a-release-manifest.json',
    ROOT / 'docs/UNIT8_F4A_JOURNEY1_STORY.md', ROOT / 'docs/UNIT8_F4A_STORY_MATRIX.md',
]
for p in required:
    if not p.exists(): problems.append(f'missing F4A artifact {p.relative_to(ROOT)}')

report = [
    '# Unit 8 F4A Narrative QA', '', '## Result', '', '**PASS**' if not problems else '**FAIL**', '',
    '## Journey 1 accounting', '',
    '- Polished scenes **12 / 12**',
    '- Locked F3 knowledge records represented **50 / 50**',
    f'- Exact-name targets explicitly introduced **{exact_targets} / 35**',
    ...
    '## Prose-quality gates', '',
    '- Every scene establishes the exact room and fixes left, center, and right geography before the biological state changes.',
    '- Dr. Mira Sen and the same transparent stimulus-response tablet persist through all twelve locations.',
    '- Every story beat preserves the exact F1 canonical scientific statement carried through the locked F3 brief.',
    '- Every exact-name target is introduced only after its defining mechanism, comparison, or consequence is visible.',
    '- Proximate versus ultimate explanation, innate versus learned contribution, sign stimulus versus fixed action pattern, migration versus cue system, kinesis versus taxis, and phototaxis versus phototropism remain diagnostically distinct.',
    '- Imprinting remains tied to a sensitive period and is not reduced to a universal first-individual rule.',
    '- Pheromone remains one chemical signal type, and receiver change remains necessary to define a signal.',
    '- Cooperation, altruism, kin selection, and inclusive fitness are framed through reproductive consequences without conscious-intention or group-good explanations.',
    '- Photoperiodism uses relative day and night length, and the hydrangea example preserves pH-mediated aluminum availability as context dependent.',
...
if not is_later_f5:
    (ROOT / 'docs/UNIT8_F4A_QA.md').write_text('\n'.join(report), encoding='utf-8')

if problems:
    print('UNIT 8 F4A QA FAIL')
    print('\n'.join(f'- {p}' for p in problems))
    sys.exit(1)

print('UNIT 8 F4A QA PASS')
print(json.dumps({
    'journey': 'U8-J1', 'scenes': 12, 'knowledge_records': 50, 'exact_name_targets': exact_targets,
