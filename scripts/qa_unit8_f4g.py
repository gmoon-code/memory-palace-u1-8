...
lock = read(U8 / 'content-lock-f4g.json')
f3_lock = read(U8 / 'content-lock-f3.json')
prior_locks = [(stage, read(U8 / f'content-lock-{stage}.json')) for stage in ('f4a','f4b','f4c','f4d','f4e','f4f')]

if j.get('palace_id') != 'U8-J7' or j.get('scene_count') != 4 or len(j.get('scenes', [])) != 4:
    problems.append('Journey 7 scene accounting invalid')
if [s['locus_id'] for s in j['scenes']] != [f'U8-L{i:02d}' for i in range(47,51)]:
    problems.append('Journey 7 locus route invalid')
ids = [kid for s in j['scenes'] for kid in s['object_ids']]
if len(ids) != 15 or len(set(ids)) != 15:
    problems.append('Journey 7 knowledge accounting invalid')
...
for s in j['scenes']:
    ...
    if not any(c['name']=='community sample card' for c in s['cast']): problems.append(f"{s['locus_id']} missing continuity cast")
    b=briefs[s['locus_id']]
...
if exact_targets != 9: problems.append(f'Expected 9 exact-name targets, got {exact_targets}')
cps=[s for s in j['scenes'] if s['checkpoint']]
if [s['locus_id'] for s in cps] != ['U8-L48','U8-L50']:
...
phrases=[
    ...
    'n in simpson diversity index'.replace('n','N',1).casefold(),
    '1 − σ(n/n)²',
    'larger values indicate greater diversity because dominance by a few species lowers the index',
    'biodiversity includes variation at genetic, species, and ecosystem levels',
...
if lock.get('lock_status') != 'LOCKED_F4G_J7' or lock.get('student_release') is not False or lock.get('preview_release') is not True:
    problems.append('F4G content-lock boundary invalid')
for rel, meta in lock.get('files', {}).items():
    p=U8 / rel if not rel.startswith('docs/') else ROOT / rel
    if not p.exists() or p.stat().st_size != meta['bytes'] or sha(p) != meta['sha256']:
        problems.append(f'F4G locked file mismatch {rel}')
...
if is_later_f5:
    ...
    if status.get('journey_count', 0) < 7 or status.get('scene_count', 0) < 50 or status.get('memory_objects') != 0 or status.get('application_challenges') != 0:
        problems.append('Unit 8 no longer preserves the F4G developer-preview boundary')

required =[*(U8/f'journeys/U8-J{i}.json' for i in range(1,8)), U8/'journeys-f4g.json', U8/'status-f4g.json', U8/'content-lock-f4g.json', U8/'f4g-release-manifest.json', ROOT/'docs/UNIT8_F4G_JOURNEY7_STORY.md', ROOT/'docs/UNIT8_F4G_STORY_MATRIX.md']
for p in required:
    if not p.exists(): problems.append(f'missing F4G artifact {p.relative_to(ROOT)}')

report=[
    '# Unit 8 F4G Narrative QA','', '## Result','', '**PASS**' if not problems else '**FAIL**','',
    '## Journey 7 accounting','',
    '- Polished scenes **4 / 4**',
    '- Locked F3 knowledge records represented **15 / 15**',
    f'- Exact-name targets explicitly introduced **{exact_targets} / 9**',
    ...
    '## Prose-quality gates','',
    '- Every scene establishes the exact observatory location and fixes left, center, and right geography before the measurement or intervention changes.',
    '- Dr. Mira Sen and one fixed community sample card persist through all four locations.',
    '- Species richness remains the number of species represented, while relative abundance remains the proportion of individuals belonging to each species.',
    '- Species diversity uses both richness and relative abundance and does not become a synonym for broader biodiversity.',
    '- Simpson diversity keeps species-specific lowercase n distinct from total community-sample capital N and preserves the AP form `1 − Σ(n/N)²`.',
    '- Larger values in the AP Simpson form are interpreted as greater diversity because dominance by a few species lowers the index.',
    '- Biodiversity remains explicitly separated into genetic, species, and ecosystem levels.',
    '- Diversity-resilience language remains conditional and does not promise greater resilience or function in every ecosystem or disturbance.',
    '- Keystone species are identified by disproportionately large community effects relative to abundance.',
    '- Foundation species remain distinct through high abundance or habitat-forming effects.',
    '- Keystone labels remain context-dependent on the ecological community and interaction network.',
    '- Student prose contains no source-management language, em dashes, or colon punctuation.',
    '- Journeys 1 through 6 remain frozen, and F1 through F3 locks remain protected.',
...
if not is_later_f5:
    (ROOT/'docs/UNIT8_F4G_QA.md').write_text('\n'.join(report), encoding='utf-8')

if problems:
    print('UNIT 8 F4G QA FAIL')
    print('\n'.join(f'- {p}' for p in problems))
    sys.exit(1)

print('UNIT 8 F4G QA PASS')
print(json.dumps({'journey':'U8-J7','scenes':4,'knowledge_records':15,'exact_name_targets':exact_targets,'optional_recalls':2,'story_words':sum(counts),'average_scene_words':round(statistics.mean(counts),1),'min_scene_words':min(counts),'max_scene_words':max(counts),'student_release':False,'preview_release':True},indent=2))
