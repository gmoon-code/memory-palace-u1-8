...
lock = read(U8 / 'content-lock-f4h.json')
f3_lock = read(U8 / 'content-lock-f3.json')
prior_locks = [(stage, read(U8 / f'content-lock-{stage}.json')) for stage in ('f4a','f4b','f4c','f4d','f4e','f4f','f4g')]

if j.get('palace_id') != 'U8-J8' or j.get('scene_count') != 8 or len(j.get('scenes', [])) != 8:
    problems.append('Journey 8 scene accounting invalid')
if [s['locus_id'] for s in j['scenes']] != [f'U8-L{i:02d}' for i in range(51,59)]:
    problems.append('Journey 8 locus route invalid')
ids = [kid for s in j['scenes'] for kid in s['object_ids']]
if len(ids) != 19 or len(set(ids)) != 19:
    problems.append('Journey 8 knowledge accounting invalid')
...
for s in j['scenes']:
    ...
    if not any(c['name']=='baseline landscape record' for c in s['cast']): problems.append(f"{s['locus_id']} missing continuity cast")
    b=briefs[s['locus_id']]
...
if exact_targets != 7: problems.append(f'Expected 7 exact-name targets, got {exact_targets}')
cps=[s for s in j['scenes'] if s['checkpoint']]
if [s['locus_id'] for s in cps] != ['U8-L52','U8-L57']:
...
phrases=[
    ...
    'overharvesting occurs when organisms are removed faster than populations can replace themselves and can cause population collapse or extinction risk',
...
if lock.get('lock_status') != 'LOCKED_F4H_J8' or lock.get('student_release') is not False or lock.get('preview_release') is not True:
    problems.append('F4H content-lock boundary invalid')
for rel, meta in lock.get('files', {}).items():
    p=U8 / rel if not rel.startswith('docs/') else ROOT / rel
    if not p.exists() or p.stat().st_size != meta['bytes'] or sha(p) != meta['sha256']:
        problems.append(f'F4H locked file mismatch {rel}')
...
if is_later_f5:
    ...
    if status.get('journey_count', 0) < 8 or status.get('scene_count', 0) < 58 or status.get('memory_objects') != 0 or status.get('application_challenges') != 0:
        problems.append('Unit 8 no longer preserves the F4H developer-preview boundary')

required =[*(U8/f'journeys/U8-J{i}.json' for i in range(1,9)), U8/'journeys-f4h.json', U8/'status-f4h.json', U8/'content-lock-f4h.json', U8/'f4h-release-manifest.json', ROOT/'docs/UNIT8_F4H_JOURNEY8_STORY.md', ROOT/'docs/UNIT8_F4H_STORY_MATRIX.md']
for p in required:
    if not p.exists(): problems.append(f'missing F4H artifact {p.relative_to(ROOT)}')

report=[
    '# Unit 8 F4H Narrative QA','', '## Result','', '**PASS**' if not problems else '**FAIL**','',
    '## Journey 8 accounting','',
    '- Polished scenes **8 / 8**',
    '- Locked F3 knowledge records represented **19 / 19**',
    f'- Exact-name targets explicitly introduced **{exact_targets} / 7**',
    ...
    '## Prose-quality gates','',
    '- Every scene establishes the exact response-center location and fixes left, center, and right geography before the environmental driver changes.',
    '- Dr. Mira Sen and one baseline landscape record persist through all eight locations.',
    '- Variation and mutation remain upstream of environmental selection. Mutation is never portrayed as arising because organisms need an adaptation.',
    '- Heterozygote advantage remains a relative-fitness relationship that depends on environmental context.',
    '- Introduced species remain distinct from invasive species. Harmful impact is required for the invasive label in the modeled context.',
    '- Habitat area loss remains distinct from isolation, and the species-area relationship remains a broad multicausal pattern.',
    '- Overharvest is tied to removal exceeding replacement over time, while global extinction-rate claims retain definitional and time-scale qualifiers.',
    '- Biological magnification remains a trophic concentration pattern for suitable persistent substances and is separated from acid deposition chemistry.',
    '- Eutrophication preserves the nutrient to production to decomposition to oxygen-demand to community-change causal chain.',
    '- Ocean acidification preserves CO₂ dissolution, carbonate-chemistry shift, lower pH, and reduced carbonate availability without promising identical biological responses.',
    '- Geological and meteorological drivers, the latitudinal diversity gradient, and disease ecology remain evidence-led and context-dependent rather than single-cause laws.',
    '- Student prose contains no source-management language, em dashes, or colon punctuation.',
    '- Journeys 1 through 7 remain frozen, and F1 through F3 locks remain protected.',
    '- All 58 F2 permanent loci now have polished F4 narrative coverage across eight journeys, covering all 211 palace-managed records.',
...
if not is_later_f5:
    (ROOT/'docs/UNIT8_F4H_QA.md').write_text('\n'.join(report), encoding='utf-8')

if problems:
    print('UNIT 8 F4H QA FAIL')
    print('\n'.join(f'- {p}' for p in problems))
    sys.exit(1)

print('UNIT 8 F4H QA PASS')
print(json.dumps({'journey':'U8-J8','scenes':8,'knowledge_records':19,'exact_name_targets':exact_targets,'optional_recalls':2,'story_words':sum(counts),'average_scene_words':round(statistics.mean(counts),1),'min_scene_words':min(counts),'max_scene_words':max(counts),'cumulative_polished_scenes':58,'cumulative_palace_records':211,'student_release':False,'preview_release':True},indent=2))
