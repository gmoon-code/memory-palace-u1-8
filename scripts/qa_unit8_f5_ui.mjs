...
const finalization=read('content/ap-biology/unit-8/finalization-f5.json');
const journeys=Array.from({length:8},(_,i)=>read(`content/ap-biology/unit-8/journeys/U8-J${i+1}.json`));

assert(status.status==='STUDENT_READY'&&status.student_release===true&&status.preview_release===false,'Unit 8 is not student-ready at F5');
assert(['UNIT8_FINALIZED_F5','UNIT8_CLASSROOM_BROWSER_VALIDATED_F6'].includes(status.pipeline_stage),'Unit 8 F5/later pipeline stage missing');
assert(status.memory_objects===211&&status.application_challenges===13,'Unit 8 F5 runtime counts drifted');
assert(registry.journey_count===8&&registry.scene_count===58&&registry.checkpoint_count===18,'Unit 8 registry drifted');
assert(finalization.unaccounted_records===0&&finalization.canonical_records===255,'Unit 8 zero-loss finalization drifted');
...
// Responsive layout contracts checked at the same desktop/tablet/phone targets used by prior F6 gates.
...
// Review scheduling, mixed discrimination, unit isolation, and five-item display cap.
...
// Shared interaction contracts inherited by the newly released unit.
...
console.log('UNIT8 F5 UI LOGIC QA PASS');
