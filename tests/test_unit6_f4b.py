...
def test_unit6_f4b_journey2_accounting_and_boundary():
    j=read(U6/'journeys/U6-J2.json')
    assert j['palace_id']=='U6-J2'
    ...
    assert [s['locus_id'] for s in j['scenes']]==[f'U6-L{i:02d}' for i in range(13,21)]
    assert len({kid for s in j['scenes'] for kid in s['object_ids']})==26

def test_unit6_f4b_prose_is_substantive_spatial_and_continuous():
    j=read(U6/'journeys/U6-J2.json')
    for s in j['scenes']:
        ...
        assert any(c['name']=='Persistent tagged gene and red RNA transcript' for c in s['cast'])

def test_unit6_f4b_exact_targets_and_story_beat_science_match_locks():
    canon={r['knowledge_id']:r for r in read(U6/'source/canonical-unit6-f1.json')['canonical_catalog']}
    briefs={b['locus_id']:b for b in read(U6/'briefs/scene-briefs-f3.json')['scene_briefs'] if b['journey_id']=='U6-J2'}
    j=read(U6/'journeys/U6-J2.json')
    exact=0
    for s in j['scenes']:
...
def test_unit6_f4b_freezes_journey1_byte_for_byte():
    lock=read(U6/'content-lock-f4b.json')
    p=U6/'journeys/U6-J1.json'
    assert hashlib.sha256(p.read_bytes()).hexdigest()==lock['frozen_f4a_journey_sha256']
    assert lock['frozen_f4a_journey_sha256']=='fe01ed27cd6a8830022f131c22bcce74f9ad094aef189c991effb66f09741b83'

def test_unit6_f4b_preview_api_without_student_release():
    ...
    if status['status']=='STUDENT_READY':
        ...
        assert client.get('/api/units/unit-6/journeys/U6-J2').status_code==200
        ...
        assert client.get('/api/units/unit-6/journeys/U6-J2').status_code==404
