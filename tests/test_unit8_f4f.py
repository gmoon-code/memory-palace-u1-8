...
def test_unit8_f4f_journey6_accounting_and_boundary():
    j = read(U8 / 'journeys/U8-J6.json')
    assert j['palace_id'] == 'U8-J6'
    ...
    assert [s['locus_id'] for s in j['scenes']] == [f'U8-L{i:02d}' for i in range(39, 47)]
    ids = [kid for s in j['scenes'] for kid in s['object_ids']]
    assert len(ids) == 29 and len(set(ids)) == 29


def test_unit8_f4f_prose_is_substantive_spatial_and_continuous():
    j = read(U8 / 'journeys/U8-J6.json')
    ...
    for s in j['scenes']:
        ...
        assert any(c['name'] == 'shared community map' for c in s['cast'])
...
def test_unit8_f4f_exact_targets_and_story_beat_science_match_locks():
...
def test_unit8_f4f_high_risk_community_distinctions_are_explicit():
    j = read(U8 / 'journeys/U8-J6.json')
    ...
    phrases = [
        ...
        'harmless or palatable species resembles a harmful or unpalatable species',
        'multiple harmful or unpalatable species converge on similar warning signals',
        ...
        'starting conditions decide the classification',
    ]
    for phrase in phrases:
        assert phrase in text


def test_unit8_f4f_only_three_first_exposure_recalls():
    j = read(U8 / 'journeys/U8-J6.json')
    cps = [s for s in j['scenes'] if s['checkpoint']]
    assert [s['locus_id'] for s in cps] == ['U8-L40', 'U8-L42', 'U8-L44']
    assert cps[0]['checkpoint_prompt'].startswith('Without looking back, identify or explain Fundamental niche')
    assert cps[1]['checkpoint_prompt'].startswith('Without looking back, identify or explain Competition')
    assert cps[2]['checkpoint_prompt'].startswith('Without looking back, identify or explain Symbiosis')


def test_unit8_f4f_prior_journeys_frozen_and_preview_hidden():
    expected = [('U8-J1',12,2),('U8-J2',9,3),('U8-J3',5,2),('U8-J4',7,2),('U8-J5',5,2)]
    ...
    if unit['status'] == 'STUDENT_READY':
        ...
        assert unit['journey_count'] >= 6 and unit['scene_count'] >= 46
        assert client.get('/api/units/unit-8/journeys').json()['guided_journeys'] == []
        for jid in ['U8-J1', 'U8-J2', 'U8-J3', 'U8-J4', 'U8-J5', 'U8-J6']:
